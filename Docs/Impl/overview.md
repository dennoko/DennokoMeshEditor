...
- **編集中の選択アウトライン／選択ワイヤーフレームの抑制**（v1.2.5 で撤去。理由は下記）

トポロジを変えないことは仕様上の重要な前提であり、後述するデータモデル（頂点インデックス基準のデルタ）の成立条件でもある。

#### 選択アウトライン抑制を撤去した理由（v1.2.5）

編集中に見えている形状は NDMF のプロキシで、元 Renderer は `forceRenderingOff` で描画されていない。
それでも選択アウトラインとワイヤーフレームは**元 Renderer の形状＝編集前の形状**で描かれるため、
編集結果の上にずれた線が重なる。これを消そうとすると、次のどちらかを選ぶことになる。

| 方式 | 問題 |
| --- | --- |
| internal API `AnnotationUtility.showSelectionOutline` を書き換える（v1.2.4 までの実装） | **Unity 全体の永続設定**（EditorPrefs）を触る。抑制中にエディタが落ちると設定が OFF のまま取り残され、ツール側の後始末コードは一切走れない。復旧は次回起動時の推測に頼るしかない。さらに Unity 6 で静かに壊れる internal API 依存を 1 件抱える |
| 公開 API `EditorUtility.SetSelectedRenderState` で対象だけ Hidden にする | 永続設定は汚さないが、選択内容が変わるたび Unity 側が既定値へ戻すため**定期的な貼り直し**が要る。さらにプロキシは毎フレーム差し替わるので、解除台帳・`SessionState` によるドメインリロード復帰・プロキシ登録の先回りフックまで必要になり、500 行規模の常駐コントローラーになった |

得られるのは「線が重ならない」という見た目の改善だけで、**編集機能そのものには寄与しない**。
一方でコストは、常駐する状態機械か、エディタ設定を壊しうる副作用のどちらかである。
割に合わないと判断して撤去した。

代替として、**Scene ビューの Gizmos メニューの "Selection Outline" をユーザー自身が切り替える**。
Unity 標準の機能であり、ツールが勝手に触るより挙動が予測しやすい。
